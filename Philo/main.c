...
/*   Updated: 2026/01/18 23:14:24 by wilisson         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "philo.h"

static int	create_philosopher_threads(t_table *table, pthread_t *threads)
{
	int	i;
	...
	while (i < table->num_philos)
	{
		if (i % 2 == 0)
		{
			if (pthread_create(&threads[i], NULL,
					odd_philo_routine, &table->philos[i]) != 0)
				return (EXIT_FAILURE);
		}
		else
		{
			if (pthread_create(&threads[i], NULL,
					non_odd_philo_routine, &table->philos[i]) != 0)
				return (EXIT_FAILURE);
		}
		i++;
	}
...
static void	join_philosopher_threads(t_table *table, pthread_t *threads)
{
	int	i;
...
int	main(int ac, char **av)
{
	...
	if (create_philosopher_threads(&table, threads) != EXIT_SUCCESS)
		return (EXIT_FAILURE);
	if (pthread_create(&table.monitor_thread,
			NULL, monitor_routine, &table) != 0)
		return (EXIT_FAILURE);
	join_philosopher_threads(&table, threads);
